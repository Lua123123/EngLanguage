...
import com.example.englanguage.databinding.FragmentFlipCard5Binding
...
class FlipCardFragment5 : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_flip_card5, container, false)
    ...
    private fun flipCart() {
        ...
        //4
        binding.cartFront4?.cameraDistance = 8000 * scale!!
        binding.cartBehind4?.cameraDistance = 8000 * scale
        ...
        //4
        binding.flipLayout4?.setOnClickListener {
            if (isFront) {
                front_anim.setTarget(binding.cartFront4)
                behind_anim.setTarget(binding.cartBehind4)
                front_anim.start()
                behind_anim.start()
                isFront = false
            } else {
                front_anim.setTarget(binding.cartBehind4)
                behind_anim.setTarget(binding.cartFront4)
                behind_anim.start()
                front_anim.start()
                isFront = true
            }
